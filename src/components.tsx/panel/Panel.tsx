...
import Task from '../task/TaskDisplay';

interface PanelProps {
  direction: PanelDirections;
  title: string;
  buttonMessage: ButtonMessages;
...
const Panel = ({
  ...
  return (
    <>
      ...
      <aside
        id="default-sidebar"
        className={`fixed top-0  z-40 w-64 h-screen transition-transform -translate-x-full sm:translate-x-0 ${direction}`}
        aria-label="Sidebar"
      >
        <div className="h-full px-3 py-4 overflow-y-auto bg-gray-50 dark:bg-gray-800 flex flex-col justify-between">
          ...
          {children}
          <GeneralButton message={buttonMessage}></GeneralButton>
        </div>
      </aside>
    </>
  );
